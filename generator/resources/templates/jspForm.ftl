          ...
          <form:form class="p-2" action="${opening_bracket}action${closing_bracket}" method="post" modelAttribute="${class_name?uncap_first}">
                <#list properties as property>
                    ...
                    <#if enum_types?seq_contains(property.type)>
                    	${label}
                        <form:select path="${property.name?uncap_first}" cssClass="form-control">
                    ...
                    <div class="form-group">
                        <form:checkbox path="${property.name?uncap_first}" />
                        ${label}
                    </div>
                    <#elseif property.type == "String" || property.type == "string" || property.type == "Long" || property.type == "long">
                    <div class="form-group">
                        ${label}
                        <form:input cssClass="form-control" path="${property.name?uncap_first}" />
                    </div>
                    <#elseif   property.type == "date" || property.type == "Date">
                    <div class="form-group">
                        ${label}
                        <form:input cssClass="form-control" type = "date" path="${property.name?uncap_first}" />
                    </div>
                    </#if>
                    </#if>
                </#list>
                    ...
                    <#if property.upper == 1 && property.oppositeEnd== -1>
                        <div class="form-group">
                            ...
                            <form:select path="${property.name}" cssClass="form-control">
                                ...
                                <form:options itemLabel="name" items="${opening_bracket}${property.name?uncap_first}List${closing_bracket}" itemValue="id"/>
                            </form:select>
                        </div>
                     <#elseif property.upper == -1>
                        <div class="form-group ">
                        ${label}
                        <form:checkboxes itemLabel="name" items="${opening_bracket}${property.name?uncap_first}List${closing_bracket}" path="${property.name}" element="div class='checkbox border rounded p-2' " itemValue="id"/>
                        </div>
                    </#if>
